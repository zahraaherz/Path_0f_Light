/**
 * Process a single book with OCR
 * Usage: node process_single_book_ocr.js <book_id>
 * Example: node process_single_book_ocr.js prophet_muhammad_vol1_002
 */
...
async function processBookWithOCR() {
  ...
  try {
    ...
    console.log(`   This will take approximately ${Math.ceil(imageFiles.length / 2)} minutes\n`);
    ...
    for (const imagePath of imageFiles) {
      ...
      if (processedPages % 50 === 0) {
        ...
        console.log(` (${elapsed}min elapsed, ~${remaining}min remaining)`);
      }
    }
